...
// TO DO NEXT: make it possible to play different instruments 
// {project code: addInstruments} 
...
class Note {
    ...
    play(duration) {
        if (instrument === 'piano') {this.playPiano(duration)}
    }
    ...
    // {project code: addInstruments} 
    playXylopohone(duration) { // like xylophone, but notes break when they overlap
    ...
    // {project code: addInstruments} 
    playGuitar(duration) { // a bit more like a distorted guitar
...
class Interval{
    ...
    updateStatsLineBarsVertical(verdictPositive){
        ...
        // TO DO: later improve the logic so that the bar flies to its own colour
        // {project code: flyToColour}
        intervalsCollection.lastBarAdded = this.statsLineBarsVertical
    }
